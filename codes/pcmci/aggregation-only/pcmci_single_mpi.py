...
def aggregate_results(
    ...
    log("AGGREGATE",
        ...
        f"min_t_data={min_t_data}  min_group_votes={min_group_votes}  "
        f"vote_weighted={vote_weighted}",
        job_start, rank)

    # key=(cause, effect, tau) → list of evidence dicts from each group
    link_evidence: dict  = {}
    total_loaded         = 0
    total_capped         = 0
    skipped_small        = 0
    ...
    for path in sorted(group_files):
        ...
        for lk in gd.get("links", []):
            ...
            if key not in link_evidence:
                link_evidence[key] = []
            link_evidence[key].append({
                "p_val":    lk["p_val"],
                "val":      lk["val"],
                "edge":     lk["edge"],
                "group_id": lk["group_id"],
                "nodes":    lk["nodes"],
                "T_data":   T_data,
            })
        ...
        if total_loaded % 100 == 0:
            log("AGGREGATE",
                f"  loaded={total_loaded}/{len(group_files)}  "
                f"unique_edges={len(link_evidence):,}",
                job_start, rank)

    log("AGGREGATE",
        f"Pass-1 done  groups_loaded={total_loaded}  "
    ...
    for (cause, effect, tau), evidence in link_evidence.items():
        ...
        if vote_weighted:
            ...
            weights  = np.array([np.sqrt(e["T_data"]) for e in evidence],
                                 dtype=np.float64)
            weights /= weights.sum()
            log_ps   = np.array([np.log(max(e["p_val"], 1e-300))
                                  for e in evidence])
            combined_p = float(np.exp((weights * log_ps).sum()))
            # representative val/edge come from the largest group
            best_idx   = int(np.argmax([e["T_data"] for e in evidence]))
        else:
            # Fix 4 – Fisher's method  (all groups equally weighted)
            chi2_stat  = -2.0 * sum(
                np.log(max(e["p_val"], 1e-300)) for e in evidence)
    ...
    sorted_links.sort(key=lambda x: x["p_val"])

    log("AGGREGATE",
        f"Pass-2 done  "
        f"candidate={len(link_evidence):,}  "
        f"fail_votes={n_fail_votes:,}  "
        f"fail_alpha={n_fail_alpha:,}  "
    ...
    with open(graph_path, "w") as fh:
        json.dump({
            ...
            "n_groups_skipped":     skipped_small,
            "n_unique_links":       len(sorted_links),
            "pc_alpha":             pc_alpha,
            "min_t_data":           min_t_data,
    ...
    for lk in sorted_links[:20]:
        log("AGGREGATE",
            ...
            f"{lk['effect']:<28}  "
            f"p_comb={lk['p_val']:.2e}  votes={lk['n_votes']}  "
            f"val={lk['val']:.4f}",
            job_start, rank)
...
def main():
    ...
    ap.add_argument("--min_group_votes", type=int,  default=2,
                    help="Fix 2: an edge must appear in at least this many "
                         "independent groups to survive (default: 2)")
    ap.add_argument("--vote_weighted",   action="store_true", default=True,
                    help="Fix 3: weight p-values by sqrt(T_data) so larger "
                         "groups dominate (default: True)")
    ap.add_argument("--no_vote_weighted", dest="vote_weighted",
                    action="store_false",
                    help="Fix 4: use unweighted Fisher chi-squared instead")
    args = ap.parse_args()
    ...
    if args.aggregate_only:
        if rank == 0:
            ...
            if group_jsons:
                aggregate_results(
                    group_jsons, output_dir, args.pc_alpha,
                    JOB_START, rank,
                    min_t_data      = args.min_t_data,
                    min_group_votes = args.min_group_votes,
                    vote_weighted   = args.vote_weighted,
                )
